...
/*!
	...
	@~japanese
		@brief デバイスの同期信号出力の遅延量をクロック単位で取得します。
		@param[in] hDevice 操作対象のデバイスハンドル
		@param[out] pDelay 遅延量(クロック単位)の格納先
		@return 成功時はPUC_SUCCEEDED、失敗時はそれ以外が返ります。
		@note 本関数はスレッドセーフです。
		@see PUC_SetSyncOutDelay
*/
...
/*!
	...
	@~japanese
		...
		@param[in] hDevice 操作対象のデバイスハンドル
		@param[out] pWidth 出力幅(クロック単位)の格納先
		@return 成功時はPUC_SUCCEEDED、失敗時はそれ以外が返ります。
		@note 本関数はスレッドセーフです。
		@see PUC_SetSyncOutWidth
*/
...
/*!
	...
	@~japanese
		...
		@details PUC_SetFramerateShutter関数で撮影速度およびシャッター速度が変更されると、本関数で返却される値も変わります。@n露光・非露光期間を変えると同期信号の出力倍率はx1倍に戻ります。
		@param[in] hDevice 操作対象のデバイスハンドル
		@param[out] pExpOnClk 露光期間（クロック単位）の格納先
		@param[out] pExpOffClk 非露光期間（クロック単位）の格納先
		@return 成功時はPUC_SUCCEEDED、失敗時はそれ以外が返ります。
		@note 本関数はスレッドセーフです。
		@see PUC_SetExposeTime
		@see PUC_SetFramerateShutter
...
/*!
	...
	@~japanese
		...
		@param[in] hDevice 操作対象のデバイスハンドル
		@param[out] pMinExpOnClk 最小露光期間（クロック単位）の格納先
		@param[out] pMinExpOffClk 最小非露光期間（クロック単位）の格納先
		@return 成功時はPUC_SUCCEEDED、失敗時はそれ以外が返ります。
		@note 本関数はスレッドセーフです。
		@see PUC_SetExposeTime
*/
